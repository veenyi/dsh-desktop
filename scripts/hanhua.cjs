...
if (fs.existsSync(CONV_FILE)) {
  ...
  // 权限下拉菜单选项汉化（Read Only / Workspace Write / Full access → 中文）
  src = src.replace(
    'return option.value === FULL_ACCESS ? "Full access" : displayName(option.name);',
    'return option.value === FULL_ACCESS ? "完全访问" : ({ "read-only": "只读", "workspace-write": "工作区写入", "full-access": "完全访问", "danger-full-access": "完全访问" }[option.name] ?? displayName(option.name));'
  );
  // 权限确认弹窗 / 等待审批 等残留英文汉化（顺序：长句 → 带问号 → 独立短语 → 通用词）
  src = src
    .replace('"Full access reduces confirmation steps and lets the agent perform more actions directly, including sensitive operations, file changes, or external commands. Only use it when you trust the current task."', '"完全访问会减少确认步骤，让 agent 直接执行更多操作（敏感操作、文件修改或外部命令）。仅建议在你信任当前任务时使用。"')
    .replace('"Enable Full access?"', '"启用完全访问？"')
    .replace('"Enable Full access"', '"启用完全访问"')
    .replace(/"Full access"/g, '"完全访问"')
    .replace('"Waiting for approval"', '"等待审批"')
    .replace(/"approval"/g, '"审批"');
  fs.writeFileSync(CONV_FILE, src);
  console.log('  permission options localized');
}
